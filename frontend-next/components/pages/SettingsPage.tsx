...
const CALLBACK_MESSAGE: Record<string, { tone: "ok" | "warn" | "bad"; text: string }> = {
  ...
  ephemeral_secret: { tone: "bad", text: "The mailbox was not connected, because this server has no AUTH_SECRET set — the credential could not be stored in a form it could still read after a restart. The authorization was handed back to Google." },
};
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    // The HASH IS PRESERVED. It carries which screen the reader is on
    // (page.tsx keeps the nav row there so a reload does not lose your
    // place), and rebuilding the URL from pathname + query alone would strip
    // it -- dropping someone back on Overview the next time they reloaded,
    ...
    <div className="mx-auto flex max-w-[900px] flex-col gap-4 p-4 sm:p-6">
      ...
      <Panel>
        ...
        <div className="mt-3">
          ...
          ) : status && !status.token_storage_durable ? (
            /* A THIRD configuration problem, with a third remedy. Shown ahead
               of the connect flow because connecting is refused server-side
               while it holds: the stored credential is encrypted with a key
               derived from AUTH_SECRET, and without one that key is generated
               per process -- so the mailbox would read Connected until the
               next restart and Access revoked afterwards. */
            <Callout tone="warn" title="No AUTH_SECRET is set on this server">
              A connected mailbox is stored encrypted under a key derived from{" "}
              <code>AUTH_SECRET</code>. With none set, that key is generated fresh on every
              restart, so the credential could not be read again afterwards and the mailbox would
              appear to have revoked itself. Set <code>AUTH_SECRET</code> in the server
              environment and restart before connecting.
            </Callout>
          ) : !connection ? (
            <EmptyState
