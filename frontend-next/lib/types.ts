...
export interface RejectionEmailActivityEntry {
  id: number;
  event_type: "REJECTION_EMAIL_SENT" | "REJECTION_EMAIL_FAILED";
  actor: string | null;
  created_at: string;
  note: string | null;
  metadata: {
...
export interface RejectionEmailPreview {
  run_id: number;
  draft: RejectionEmailDraft;
  sender: { available: boolean; reason: string | null };
  history: RejectionEmailActivityEntry[];
  already_sent: boolean;
}

export interface RejectionEmailSendResult {
  ok: boolean;
  recipient: string;
  subject: string;
  message_id: string | null;
